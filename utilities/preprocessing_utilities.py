import ast, argparse, pickle, time
import networkx as nx

hierarchical_relation_preps = ["during", "in", "amid", "throughout", "including", "within"]
hierarchical_relation_ev12ev2_preps = ["including"]
temporal_relation_preps = ["following", "before", "after"]
temporal_relation_ev12ev2_preps = ["before"]
causal_relation_preps = ["lead_to", "result_in", "result_from", "cause", "cause_by", "because_of",
                            "due_to", "trigger", "triggered_by"]
causal_relation_ev12ev2_preps = ["lead_to", "result_in", "cause", "trigger"]

hierarchical_relation_advcls = ["while", "when"]
temporal_relation_advcls = []
causal_relation_advcls = []
...
def detect_event_trigger(sentence, idx):
    ...
    if sentence.POSList[idx][0] == "V":
        ...
        #     return False
        if sentence.lemmaList[idx] in event_noun_set:
            return True
        else:
            return False
    else:
...
def validate_eventpair(eventpair):
    ...
    # Removing restrictions by Yao et al on stock related events
