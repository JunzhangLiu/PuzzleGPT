...
genai.configure(api_key="AIzaSyDXBf2XjY9-20lmNq6S92P3DNkiN2aiA8Y")
...
class ImagePatch:
    ...
    simple_query(question: str=None, model: str='Gemini')->str
    ...
    def __init__(self, image: Union[Image.Image, torch.Tensor, np.ndarray], left: int = None, lower: int = None,
        ...
        if left is None and right is None and upper is None and lower is None:
            ...
            if raw_image is not None:
                self.raw_cropped_image = raw_image
            else:
                self.raw_cropped_image = transforms.ToPILImage()(self.cropped_image).convert("RGB")
            self.left = 0
            self.lower = 0
    ...
    def score(self, category: str) -> float:
        ...
        model_name = 'clip'
        # print('good so far')
        image2compare = self.cropped_image
        text = category
        score = self.forward(model_name, image2compare, text, task='text score')
    ...
    def best_image_match(self, image: torch.Tensor):
        """Return the similarity score between two images.
        Parameters
        -------
        image : torch.Tensor
            A torch.Tensor containing the image to compare with the main patch
        """
        ...
        image2compare = self.cropped_image
        image = image
        score = self.forward(model_name, image2compare, image, task='image score')
        return score

    def simple_query(self, question: str, model: str = 'LLaVA'):
        """Returns the answer to a basic question asked about the image. If no question is provided, returns the answer
        ...
        if 'blip' in model.lower():
            return self.forward('blip', self.raw_cropped_image, question, task='qa')
        elif 'gemini' in model.lower():

            gemini = genai.GenerativeModel(model_name="gemini-1.5-flash-latest")
            if isinstance(self.raw_cropped_image, torch.tensor):
                img = transforms.ToPILImage()(self.raw_cropped_image).convert("RGB")
            else:
                img = self.raw_cropped_image
            response = gemini.generate_content([question, img])
            return response.text
        else:
            return self.forward('LLaVA', self.raw_cropped_image, question=question, task='qa')
    ...
    def read_text(self):
        ...
        question : str
            A string describing the question to be asked.
        """
        return self.simple_query('What are the text shown on this photograph?', 'LLaVA')
    ...
    def recognize_text(self):
        ...
        question : str
            A string describing the question to be asked.
        """
        return self.forward('ocr', self.raw_cropped_image, task='rec')
    ...
    def print_and_save_image(self, save_path: str):
        try:
            show_single_image(self.cropped_image, save_path=save_path)
        except:
            pass
