...
class CLIPModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, version="ViT-L/14@336px"):  # @336px
        ...
        with HiddenPrints('CLIP'):
            model, preprocess = clip.load('ViT-B/32', device=self.dev)
    ...
    @torch.no_grad()
    def image_score(self, image: torch.Tensor, image2: torch.Tensor):

        image = image.unsqueeze(0).to(self.dev)
        image2 = image2.unsqueeze(0).to(self.dev)

        image_features = self.model.encode_image(image).float()
        image_features /= image_features.norm(dim=-1, keepdim=True)
        image_features2 = self.model.encode_image(image2).float()
        image_features2 /= image_features2.norm(dim=-1, keepdim=True)

        sim_logits = 100.0 * image_features @ image_features2.T
        return sim_logits

    @torch.no_grad()
    def text_score(self, image, text: str):

        image = image.unsqueeze(0).to(self.dev)
        text_emb = self.clip.tokenize(text).to(self.dev)

        image_features = self.model.encode_image(image).float()
        image_features /= image_features.norm(dim=-1, keepdim=True)
        text_features = self.model.encode_text(text_emb).float()
        text_features /= text_features.norm(dim=-1, keepdim=True)

        sim_logits = 100.0 * image_features @ text_features.T
        return sim_logits
    ...
    def forward(self, image, prompt, task='score', return_index=True, negative_categories=None, return_scores=False):
        if task == 'classify':
            ...
            out = clip_score
        elif task == 'text score':
            clip_score = self.text_score(image=image, text=prompt)
            out = clip_score
        elif task == 'image score':
            clip_score = self.image_score(image=image, image2=prompt)
            out = clip_score
        else:  # task == 'compare'
...
class BLIPModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, half_precision=config.blip_half_precision,
        ...
        self.qa_prompt = "Question: {} Answer:"
    ...
    def caption(self, image, prompt=None):
        raw_image = transforms.ToPILImage(image).convert("RGB")
        image = self.vis_processors["eval"](raw_image).unsqueeze(0).to(self.dev)
        if prompt is not None:
            answer = self.model.generate({"image": image, "prompt": prompt})
        else:
            answer = self.model.generate({"image": image, "prompt": "Question: Simple describe the image? Answer:"})
    ...
    def qa(self, image, question):
        ...
        if 'answer' in question.lower():
            answer = self.model.generate({"image": image, "prompt": f"Question: {question}"})
        else:
    ...
    def forward(self, image, question=None, task='caption'):
        ...
        ###
        if task == 'qa':
            response = self.qa(image, question)
        else:
            response = self.caption(image)
        ###
        return response
...
class OCR(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        with warnings.catch_warnings(), HiddenPrints("OCR"), torch.cuda.device(self.dev):
            self.ocr = MMOCRInferencer(det='dbnetpp', rec='satrn')
            self.rec = MMOCRInferencer(rec='satrn')

    @torch.no_grad()
    def recognize(self, image, task='det_rec'):
        if not isinstance(image, Image.Image):
            raw_image = transforms.ToPILImage(image).convert("RGB")
        else:
            raw_image = image
        if task == 'det_rec':
            return self.ocr(np.array(raw_image), show=False, print_result=False)['predictions'][0]['rec_texts']
        elif task == 'rec':
            return self.rec(np.array(raw_image), show=False, print_result=False)['predictions'][0]['rec_texts']

    def forward(self, image, task='det_rec'):
        ###
        response = self.recognize(image, task=task)
        ###
        return response
...
class LLaVAModel(BaseModel):
    ...
    def __init__(self, gpu_number=1):
        ...
        with warnings.catch_warnings(), HiddenPrints("LLaVA"), torch.cuda.device(self.dev):
            try:
    ...
    def qa(self, image, question):
        ...
        # raw_image.show()
        prompt = f"USER: <image>\n{question} ASSISTANT:"
        inputs = self.processor(text=prompt, images=raw_image, return_tensors="pt").to(self.dev)
        generate_ids = self.model.generate(**inputs, max_new_tokens=self.max_words)
        output = \
    ...
    def forward(self, image, question=None, task='qa'):
        """
        if not self.to_batch:
            image, question, task = [image], [question], [task]
